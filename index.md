...
<div class="posts">
 ...
 {% if post.description.size > 140 %}{{ post.description | markdownify | remove: '<p>' | remove: '</p>' }}{% else %}{{ post.excerpt | markdownify | remove: '<p>' | remove: '</p>' }}{% endif %} <a href="{{ site.url }}{{ post.url }}" title="继续阅读"><strong>继续阅读...</strong></a>
  </div>
  <hr class="transp">
  {% endfor %}
</div>
